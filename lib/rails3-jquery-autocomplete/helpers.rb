      ...
      case implementation
        ...
        when :activerecord
		  ...
		  unless options[:word]
		    ...
		    words = term.downcase.split(' ').collect do |word| "%#{word}%" end
			querypart = " AND LOWER(#{method}) LIKE ?"
			items = relation.where("LOWER(#{method}) LIKE ?" + querypart * (words.length - 1), *words)
          end
      end
    end
  end
